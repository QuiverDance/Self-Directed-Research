...
def _kivi_get_past_length(past):
    if past is None:
        return 0
    # Old-style: list/tuple of layer tuples → last item stores kv_seq_len in KIVI
    try:
        return past[0][-1]
    except Exception:
        # HF 4.43+ Cache object
        try:
            return past.get_seq_length()
        except Exception:
            return 0

def _kivi_has_nonempty_past(past):
    try:
        return _kivi_get_past_length(past) > 0
    except Exception:
        return False
# --- end helpers ---
